...
class SubGrapher:	
	...
	def plotHist(self, stats, data, xlabel, ylabel):			
		self.axes.clear()
		if stats:
			  self.__printStats()
	...
	def plotHistSize(self, stats):
		if self.data == None:
			self.__prepareData
		self.plotHist(stats, self.data['size'], "request size (kiB)", "count") 
		
	def plotHistTime(self, stats):
		if self.data == None:
			self.__prepareData
			
		self.plotHist(stats, self.data['dur'], "request duration (ms)", "count")												

	def plotHistSpeed(self, stats):
		if self.data == None:
			self.__prepareData
			
		self.plotHist(stats, self.data['size']/self.data['dur']*1000, "request speed (kiB/s)", "count")
